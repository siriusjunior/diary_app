class ChatroomsController < ApplicationController
  ...
  def create
    users = User.where(id: params.dig(:chatroom, :user_id)) + [current_user]
  ...
  private
    ...
    def require_user_ids
      redirect_back(fallback_location: root_path, danger: 'パラメーターが不正です') if params.dig(:chatroom, :user_id).reject(&:blank?).blank?
    end
